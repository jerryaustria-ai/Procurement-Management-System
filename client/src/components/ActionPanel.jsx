...
function getAdvanceLoadingLabel(currentStage, hasApprovalAttachment) {
  if (
    (currentStage === 'Review' || currentStage === 'Approval') &&
    hasApprovalAttachment
  ) {
...
const STAGE_DESCRIPTIONS = {
  ...
  'Request for Payment':
    'Finalize the payment request details before approval.',
...
export default function ActionPanel({
  ...
  const supplierFieldValue =
    form.supplier === 'Pending selection' ? '' : form.supplier
    ...
    <section className='panel action-panel panel-with-expand'>
            ...
            <div className='stage-field po-breakdown-section'>
              ...
              <div className='po-totals-grid'>
                <label>
                  Sub Total
                  <input
                    value={formatAmount(poSubTotal, item.currency)}
                    readOnly
                  />
                </label>
                <label>
                ...
                <label className='po-net-total'>
                  Net Total
                  <input
                    value={formatAmount(poNetTotal, item.currency)}
                    readOnly
                  />
                </label>
              </div>
          ...
          <div className='approval-upload-section'>
            ...
            {uploadForm.file ? (
              <div className='approval-upload-file-pill'>
                <span className='approval-upload-file-name'>
            ...
            {!uploadForm.file ? (
              <p className='panel-support'>
                Upload the attachment that proves this purchase request was
                approved.
              </p>
            ) : (
              <p className='panel-support'>
                The selected file will be uploaded automatically when you click
                Approve.
              </p>
            )}
          </div>
      ...
      {!canAdvance ? (
        <p className='error-text'>
          Your role cannot advance the current stage.
        </p>
      ) : null}
      {isRejectedWorkflow ? (
        <p className='error-text'>
          This request has been rejected and can no longer continue.
        </p>
      ) : null}
      {error ? <p className='error-text'>{error}</p> : null}

      {(showSupplierSearchField || showSupplierPickerField) &&
      isSupplierModalOpen ? (
        <Modal
