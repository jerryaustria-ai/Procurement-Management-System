...
import { EquipmentUserSelect, useEquipmentUsers } from './EquipmentUserSelect.jsx'
...
function Field({ label, type = 'text', value, onChange, disabled, required, companies, directory }) {
  if (type === 'user') return <EquipmentUserSelect {...{ label, value, onChange, required, disabled, directory }} />
...
export function EquipmentForm({ form, setForm, onSubmit, onClose, editing, busy, error, companies, directoryUrl, token }) {
  ...
  return <div className='inventory-modal-backdrop'><form className='inventory-modal' onSubmit={onSubmit}>
    ...
    <fieldset disabled={busy} className='equipment-fieldset'><div className='inventory-form-grid'>{EQUIPMENT_FIELDS.map((field) => <Field key={field[0]} label={field[0] === 'recordCode' && !editing ? 'Asset Tag or Inventory Number (auto-generated)' : field[1]} directory={directory} type={field[0] === 'accountableTo' ? 'user' : field[0] === 'company' ? 'company' : field[2]} companies={companies} value={equipmentValue(form, field)} required={['name', 'category', 'condition', 'status'].includes(field[0])} disabled={!editing && field[0] === 'recordCode' || !!editing && ['status', 'accountableTo', 'department', 'issueDate'].includes(field[0])} onChange={(value) => setField(field, value)} />)}</div>
    {!editing && form.accountableTo ? <><h3>Initial accountability</h3><div className='inventory-form-grid'>{[['issuedBy', 'Issued By'], ['receivedBy', 'Received By'], ['transferDetails', 'Transfer Details'], ['remarks', 'Accountability Remarks']].map(([key, label]) => <Field key={key} label={label} directory={directory} type={['issuedBy', 'receivedBy'].includes(key) ? 'user' : 'text'} required={['issuedBy', 'receivedBy'].includes(key)} value={form.initialAccountability?.[key] || ''} onChange={(value) => setForm((current) => ({ ...current, initialAccountability: { ...current.initialAccountability, [key]: value } }))} />)}</div></> : null}
...
export function EquipmentDetails({ item, canEdit, onClose, onEdit, onArchive, onMovement, directoryUrl, token }) {
  const directory = useEquipmentUsers(directoryUrl, token)
